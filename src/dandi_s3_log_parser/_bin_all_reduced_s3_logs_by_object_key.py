    ...
    *,
    ...
    for reduced_s3_log_file in reduced_s3_log_files:
        ...
        for _, row in binned_data_frame.iterrows():
            object_key_as_path = pathlib.Path(row.name)
            object_key_as_path.parent.mkdir(parents=True, exist_ok=True)
            ...
            row.to_csv(path_or_buf=binned_s3_log_file_path, mode="a", sep="\t", header=header)
