...
def _map_reduced_logs_to_dandiset(
    ...
    for version in dandiset.get_versions():
        ...
        for asset in dandiset_version.get_assets():
            ...
            blob_or_zarr = "blob" if ".zarr" not in asset_suffixes else "zarr"

            reduced_log_file_path = reduced_s3_log_folder_path / f"{blob_or_zarr}_{asset_id}"

            if not reduced_log_file_path.exists():
                continue  # No reduced logs found (possible asset was never accessed); skip to next asset

            reduced_log = pandas.read_table(filepath_or_buffer=reduced_log_file_path, index_col=0)
        ...
        mapped_log = pandas.concat(objs=all_reduced_logs)
