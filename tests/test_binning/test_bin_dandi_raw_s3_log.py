...
def test_reduce_dandi_raw_s3_log_example_0(tmpdir: py.path.local) -> None:
    ...
    for test_reduced_s3_log_file_path in test_output_file_paths:
        assert (
            test_reduced_s3_log_file_path.stem in expected_asset_ids
        ), f"Asset ID {test_reduced_s3_log_file_path.stem} not found in expected asset IDs!"

        is_asset_zarr = "zarr" in str(test_reduced_s3_log_file_path)
        if is_asset_zarr:
            blob_id = test_reduced_s3_log_file_path.stem
            expected_parsed_s3_log_file_path = expected_reduced_s3_logs_folder_path / "zarr" / f"{blob_id}.tsv"
        else:
            blob_id = test_reduced_s3_log_file_path.stem
        ...
        pandas.testing.assert_frame_equal(left=test_reduced_s3_log, right=expected_parsed_s3_log)
